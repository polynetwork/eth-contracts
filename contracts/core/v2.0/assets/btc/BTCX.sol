...
contract BTCX is ERC20Extended {
    ...
    */
    function lock(uint64 toChainId, bytes memory toUserAddr, uint64 amount) public returns (bool) {
        ...
        require(eccm.crossChain(toChainId, contractAddrBindChainId[toChainId], "unlock", txData), "EthCrossChainManager crossChain executed error!");

        emit LockEvent(address(this), _msgSender(), toChainId, contractAddrBindChainId[toChainId], toUserAddr, amount);
