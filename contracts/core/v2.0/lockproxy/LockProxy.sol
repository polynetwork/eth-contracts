...
import "./../CrossChainManager/interface/IEthCrossChainManager.sol";
import "./../CrossChainManager/interface/IEthCrossChainManagerProxy.sol";
...
contract LockProxy is Context {
    ...
    */
    ...
    function test_serializeTxArgs(bytes memory toAssetHash, bytes memory toAddress, uint256 amount) public returns (bytes memory) {
        TxArgs memory txArgs = TxArgs({
            toAssetHash: toAssetHash,
            toAddress: toAddress,
            amount: amount
    ...
    function test_deserializeTxArgs(bytes memory txData) public returns (address, address, uint256) {
        TxArgs memory args = _deserializTxArgs(txData);
        address toAssetHash = Utils.bytesToAddress(args.toAssetHash);
        address toAddress = Utils.bytesToAddress(args.toAddress);
        return (toAssetHash, toAddress, args.amount);
