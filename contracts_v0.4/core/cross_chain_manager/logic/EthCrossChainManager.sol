...
contract EthCrossChainManager is IEthCrossChainManager, UpgradableECCM {
    ...
    *  @return              true or false
    */
    function crossChain(uint64 toChainId, bytes toContract, bytes method, bytes txData) whenNotPaused external returns (bool) {
        // Load Ethereum cross chain data contract
        IEthCrossChainData eccd = IEthCrossChainData(EthCrossChainDataAddress);
    ...
    */
    function verifyHeaderAndExecuteTx(bytes memory proof, bytes memory rawHeader, bytes memory headerProof, bytes memory curRawHeader,bytes memory headerSig) whenNotPaused public returns (bool){
        ...
        if (header.height >= curEpochStartHeight) {
            ...
            require(ECCUtils.getHeaderHash(rawHeader) == Utils.bytesToBytes32(ECCUtils.merkleProve(headerProof, curHeader.blockRoot)), "verify header proof failed!");
        }
    ...
    */
    function _executeCrossChainTx(address _toContract, bytes memory _method, bytes memory _args, bytes memory _fromContractAddr, uint64 _fromChainId) internal returns (bool){
        ...
        require(_toContract.call(abi.encodePacked(bytes4(keccak256(abi.encodePacked(_method, "(bytes,bytes,uint64)"))), abi.encode(_args, _fromContractAddr, _fromChainId))), "EthCrossChain call business contract failed");
